...
$ ->
	...
	$form.find '.profile.submit-button' .click ->
		...
		(data) <- $.ajax "#{CONFIG.urls.api}/account/name/update" {
			data:
				'name': $form.find '.profile.name' .val!}
		.done!

		(data) <- $.ajax "#{CONFIG.urls.api}/account/comment/update" {
			data:
				'comment': $form.find '.profile.comment' .val!}
		.done!

		(data) <- $.ajax "#{CONFIG.urls.api}/account/url/update" {
			data:
				'url': $form.find '.profile.url' .val!}
		.done!

		(data) <- $.ajax "#{CONFIG.urls.api}/account/location/update" {
			data:
	...
	$form.find '.apps > .app' .each ->
		...
		$app.find \.remove .click ->
			...
			$.ajax "#{CONFIG.urls.api}/account/remove-app" {
				data: fd
				data-type: \json}
			.done (data) ->
				$app.remove!
...
function init-checkbox-controll $checkbox
	$checkbox.change ->
		...
		$.ajax "#{CONFIG.urls.api}/web/user-settings/update" {data}
		.always ->
			$checkbox
				..attr \disabled off
